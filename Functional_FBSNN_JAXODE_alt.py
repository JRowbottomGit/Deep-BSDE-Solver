...
def XYZpaths(params, t, W, X0):
    ...
    dy_tilde = phi_tf(t0, X0, Y0, Z0) * (dt) + jnp.dot(jnp.dot(Z0.T, sigma_tf(t0, X0, Y0)), dW)
    Y0_tilde = jnp.asarray([0.])  # never used
    initial = (t0, X0, Y0, Y0_tilde, Z0, dy, dy_tilde)
    def body(carry, tW):
        dt, dW = tW
        t0, X0, Y0, Y0_tilde, Z0, dy0, dy_tilde0 = carry
        dx = mu_tf(t0, X0, Y0, Z0) * (dt) + jnp.dot(sigma_tf(t0, X0, Y0), dW)  ##dx step
    ...
    t_trace, X_trace, Y_trace, Y_tilde_trace, Z_trace, dy_trace, dy_tilde_trace = trace
    t_end, X_end, Y_end, Y_tilde_end, Z_end, dy_end, dy_tilde_end = final_state
    ...
    DY = jnp.concatenate((dy_trace, jnp.reshape(dy_end,(1,1))), 0)
    DY_tilde = jnp.concatenate((dy_tilde_trace, jnp.reshape(dy_tilde_end,(1,1))), 0)
    return X, Y, Y_tilde, Z, DY, DY_tilde
...
def loss_function(params, t, W, Xzero):  # idea take M,D out by making X0
    X, Y, Y_tilde, Z, DY, DY_tilde = vXYZpaths(params, t, W, Xzero)
    loss = jnp.sum(jnp.power(Y[:, 1:-1, :] - Y_tilde[:, 1:-1, :], 2))
    loss += jnp.sum(jnp.power(DY[:, 1:, :] - DY_tilde[:, 1:, :], 2)) #new term
...
if __name__ == "__main__":
    ...
    X_pred, Y_pred, Y_tilde_pred, Z, DY_pred, DY_tilde_pred = vXYZpaths(params, t_test, W_test, Xzero)
