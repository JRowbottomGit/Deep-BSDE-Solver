...
def hess_forward(params, t, X):
    hessu = jax.jacfwd(jax.jacrev(forward, argnums=(2)), argnums=(2))
    HU = hessu(params, t, X)
    return HU
...
def XYZpaths(params, t, W, X0):
    ...
    Hess0 = hess_forward(params, t0, X0)
    # dW = W[0,:]##EULER STEP
    # dx = mu_tf(t0, X0, Y0, Z0) * (dt) + jnp.dot(sigma_tf(t0, X0, Y0), dW)  ##dx step
    # dy = dt_forward(params, t0, X0) + jnp.dot(Z0, dx) ##EULER STEP
    Y0_tilde = jnp.asarray([0.])  # never used
    # initial = (t0, X0, Y0, Y0_tilde, Z0, Hess0)
    # initial = (t0, X0, Y0, Y0_tilde, Z0, dy)
    initial = (t0, X0, Y0, Y0_tilde, Z0, Hess0, jnp.asarray([0.]), jnp.asarray([0.]))
    def body(carry, tW):
        dt, dW = tW
        # t0, X0, Y0, Y0_tilde, Z0, dy0 = carry
        # t0, X0, Y0, Y0_tilde, Z0, Hess0 = carry
        t0, X0, Y0, Y0_tilde, Z0, Hess0, sHs0, trace0 = carry
        dx = mu_tf(t0, X0, Y0, Z0) * (dt) + jnp.dot(sigma_tf(t0, X0, Y0), dW)  ##dx step
        ...
        ##########################################################
        sig = sigma_tf(t0, X0, Y0)
        f_partial = partial(forward, params, t0)
        def hvp(f_partial, primals, tangents):
            # return jvp(grad(f_partial), primals, tangents)[1]
            return jvp(grad(f_partial), (primals,), (tangents,))[1]

        # hvp0 = hvp(f_partial, primals=(X0,), tangents=(sig[0,:],))
        # hvp0 = hvp(f_partial, primals=X0, tangents=sig[0,:])

        vhvp = vmap(hvp, in_axes=(None,None,0))
        vhvp0 = vhvp(f_partial, X0, sig)
        # sHs = jnp.dot(sig[0,:], hvp0)
        vdot = vmap(jnp.dot, in_axes=(0,0))
        vsHs = vdot(sig, vhvp0)
        sumvsHs = jnp.asarray([jnp.sum(vsHs)])
        trace = jnp.asarray([jnp.trace(jnp.dot(sigma_tf(t0, X0, Y0).T, jnp.dot(Hess0, sigma_tf(t0, X0, Y0))))])
        ##########################################################
        wah = sumvsHs + trace
        # ddydx = 0.5 * jnp.trace(jnp.dot(sigma_tf(t0, X0, Y0).T,jnp.dot(Hess0,sigma_tf(t0, X0, Y0))))
        ddydx = 0.5 * sumvsHs
        ...
        Hess1 = hess_forward(params, t1, X1)
        # carry_new = t1, X1, Y1, Y1_tilde, Z1, dy
        carry_new = t1, X1, Y1, Y1_tilde, Z1, Hess1, trace, sumvsHs
        return (carry_new, carry)

    final_state, trace = lax.scan(body, initial, (t, W))  ###the sauce

    # t_trace, X_trace, Y_trace, Y_tilde_trace, Z_trace, Hess_trace = trace
    # t_end, X_end, Y_end, Y_tilde_end, Z_end, Hess_end = final_state
    t_trace, X_trace, Y_trace, Y_tilde_trace, Z_trace, Hess_trace, trace_trace, sHs_trace = trace
    t_end, X_end, Y_end, Y_tilde_end, Z_end, Hess_end, trace_end, sHs_end = final_state
    ...
    Hess = jnp.concatenate((Hess_trace, jnp.reshape(Hess_end, (1, D, D))), 0)
    Trace = jnp.concatenate((trace_trace, jnp.reshape(trace_end, (1, 1))), 0)
    SHS = jnp.concatenate((sHs_trace, jnp.reshape(sHs_end, (1, 1))), 0)

    return X, Y, Y_tilde, Z, Hess, Trace, SHS
...
def loss_function(params, t, W, Xzero):  # idea take M,D out by making X0
    ...
    X, Y, Y_tilde, Z, Hess, Trace, SHS = vXYZpaths(params, t, W, Xzero)
...
if __name__ == "__main__":
    ...
    X_pred, Y_pred, Y_tilde_pred, Z, Hess, Trace, SHS = vXYZpaths(params, t_test, W_test, Xzero)
