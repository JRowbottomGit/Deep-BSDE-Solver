...
def forward(params, t, X):
    ...
    return jnp.reshape(u,())

vforward = vmap(forward, in_axes=(None, 0, 0))

def grad_forward(params, t, X):
    gradu = grad(forward,argnums=(1)) #<wrt X not params or t
...
# TypeError: Gradient only defined for scalar-output functions. Output had shape: (100, 1)
...
#could also vmap this but needs to correct indices, ie remove 0th M dimension
def XYpaths(params, t, W, Xzero):
    ...
    Y0 = vforward(params, t0, X0)

    Z0 = vgrad_forward(params, t0, X0)
    ...
    for n in range(0, N):
        t1 = t[:, n + 1, :]
        W1 = W[:, n + 1, :]
        sig = vsigma_tf(t0, X0, Y0)
        print(f"sigxy {sig.shape}")

        X1 = X0 + mu_tf(t0, X0, Y0, Z0) * (t1 - t0) + jnp.dot(vsigma_tf(t0, X0, Y0), (W1 - W0))
        Y1_tilde = Y0 + phi_tf(t0, X0, Y0, Z0) * (t1 - t0) + jnp.dot(jnp.dot(Z0.T, vsigma_tf(t0, X0, Y0)),
                                                                          (W1 - W0))
        print(f"t1 {t1.shape}")
        print(f"X1 {X1.shape}")
        Y1 = vforward(params, t1, X1)
        Z1 = vgrad_forward(params, t1, X1)
    ...
    Y_tilde = jnp.stack(Y_tilde_list, dim=1)

    return X, Y, Y_tilde


# jit static arg nums (0,1,2,3,)
def loss_function(params, M, D, N, t, W, Xzero):   #idea take M,D out by making X0
    loss = 0
    X,Y,Y_tilde = XYpaths(params, t, W, Xzero)
...
def train(T,M,N,D, params, N_Iter, learning_rate, Xzero):
    ...
    for it in range(previous_it, previous_it + N_Iter):
        ...
        loss = loss_function(params, M, D, N, t_batch, W_batch, Xzero)  #annoying as it gets calc'd twice
        ...
        graph = jnp.stack((iteration, training_loss))
    return params, graph
...
def run_model(T,M,N,D, init_params, Xzero, N_Iter, learning_rate):
    ...
    X_pred, Y_pred, Y_tilde_pred = XYpaths(params, t_test, W_test, Xzero)
